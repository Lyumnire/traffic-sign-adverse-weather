"""
Traffic Sign Classification under Adverse Weather - Inference Script (v4)
========================================================================
...
v4 Changes:
- Added weighted soft voting (gives higher weight to top-performing ConvNeXt V2)
- Added USE_TTA toggle (allows turning off multi-scale TTA for 2x speedup)
- Added ENABLED_MODELS flags (allows toggling models easily)
- Removed PyTorch JIT compilation (proven to slow down inference on CPU)
"""
...
# Ensemble Tuning Controls
USE_TTA = True  # Set to False to speed up inference by 2x (disables multi-scale TTA)

# Weights based on validation/testing performance
# ConvNeXt V2 (0.938) gets highest weight; Swin V2 (0.916) and EffNetV2 (0.917) get lower weights
MODEL_WEIGHTS = {
    "convnextv2_base":   0.60,
    "swin_v2_b":         0.20,
    "efficientnet_v2_m": 0.20,
}

# Toggle models easily on the platform
ENABLED_MODELS = {
    "convnextv2_base":   True,
    "swin_v2_b":         True,
    "efficientnet_v2_m": True,
}
...
def _load_all():
    ...
    with _load_lock:
        ...
        for name, builder, weight_file, default_size in MODEL_SPECS:
            if not ENABLED_MODELS.get(name, True):
                print(f"[INFO] Model {name} is disabled in config, skipping.", file=sys.stderr)
                continue
            ...
            try:
                ...
                if missing_keys or unexpected_keys:
                    print(f"[ERROR] Architecture mismatch for {name}!", file=sys.stderr)
                    print(f"  Model keys: {len(model_keys)}, Checkpoint keys: {len(ckpt_keys)}", file=sys.stderr)
                    print(f"  Missing keys: {len(missing_keys)}, Unexpected keys: {len(unexpected_keys)}", file=sys.stderr)
                    ...
                    print(f"  SKIPPING this model to avoid random predictions.", file=sys.stderr)
                    continue

                model.load_state_dict(cleaned, strict=True)
                model.to(DEVICE).eval()
                _models.append((name, model, int(input_size), False))
                print(f"[OK] PyTorch: {name} (input_size={input_size}, keys={len(model_keys)})", file=sys.stderr)
            except Exception as e:
...
def _infer_model_tta(name, model_or_sess, input_size, is_onnx, bgr_img):
    """Run model with optional multi-scale TTA. NO horizontal flip."""
    infer_fn = _infer_onnx if is_onnx else _infer_pytorch

    # Primary scale
    p1 = infer_fn(model_or_sess, bgr_img, input_size)

    # Secondary scale (only if TTA is enabled)
    if USE_TTA:
...
def predict(X):
    ...
    # Ensemble: weighted soft voting across all available models
